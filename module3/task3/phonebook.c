...
void load_book(PhoneBook* book, const char* filename) {
    ...
    Entry temp;
    while (1) {
        ssize_t r = read(fd, &temp, sizeof(Entry));
        if (r == 0) break;
        if (r < 0) break;
        if (r != sizeof(Entry)) break;

        int ident = temp.ident;
...
void save_book(PhoneBook* book, const char* filename) {
    int fd = open(filename, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0) return;

    for (int i = 0; i < MAXENTRY; i++) {
        if (book->mask[i / 8] & (1 << (i % 8))) {
            write(fd, &book->entries[i], sizeof(Entry));
        }
    }
